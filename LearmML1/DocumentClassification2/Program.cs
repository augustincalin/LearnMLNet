...
string _connectionString = "Data Source=.;Initial Catalog=COPDB;User Id=sa;Password=safz_edekk1";
...
string sqlCommandTrain = "SELECT * FROM Documents_2 WHERE IsTest = 0";
// string sqlCommandTrain = "SELECT * FROM Documents2";
string sqlCommandTest = "SELECT * FROM Documents_2 WHERE IsTest = 1";
...
{
    var pipeline = _mlContext.Transforms.Categorical.OneHotEncoding(@"UseCase", @"UseCase")
                                    .Append(_mlContext.Transforms.Text.FeaturizeText(inputColumnName: @"RawText", outputColumnName: @"RawText"))
                                    .Append(_mlContext.Transforms.Concatenate(@"Features", new[] { @"UseCase", @"RawText" }))
                                    .Append(_mlContext.Transforms.Conversion.MapValueToKey(outputColumnName: @"Category", inputColumnName: @"Category"));
    return pipeline;
}
...
IEstimator<ITransformer> BuildAndTrainModel(IDataView trainingDataView, IEstimator<ITransformer> pipeline)
{
    var trainingPipeline = pipeline
        .Append(_mlContext.MulticlassClassification.Trainers.OneVersusAll(binaryEstimator: _mlContext.BinaryClassification.Trainers.FastTree(new FastTreeBinaryTrainer.Options() { NumberOfLeaves = 4, MinimumExampleCountPerLeaf = 20, NumberOfTrees = 4, MaximumBinCountPerFeature = 256, FeatureFraction = 1, LearningRate = 0.1, LabelColumnName = @"Category", FeatureColumnName = @"Features" }), labelColumnName: @"Category"))
